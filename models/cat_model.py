...
class CatModel(BaseModel):
    ...
    def __init__(self, opt):
        super(CatModel, self).__init__(opt)
    ...
    def forward(self):
        ...
        if self.isTrain or (not self.isTrain and not self.opt.chop):
            # datat.raws: [N, T, C, H, W],在第二个维度进行展开，分为T个数据,每个数据为[N,C, H, W]
            # x1 = self.data_raws[:, 0, :, :, :]
            # x2 = self.data_raws[:, 1, :, :, :]
            # x3 = self.data_raws[:, 2, :, :, :]
            # x4= self.data_raws[:, 3, :, :, :]
            # x5 = self.data_raws[:, 4, :, :, :]
            # x1=torch.cat((x1,ldr_to_hdr(x1,1,1/2.2)),1)
            # x2=torch.cat((x2,ldr_to_hdr(x2,1,1/2.2)),1)
            # x3=torch.cat((x3,ldr_to_hdr(x3,1,1/2.2)),1)
            # x4=torch.cat((x4,ldr_to_hdr(x4,1,1/2.2)),1)
            # x5=torch.cat((x5,ldr_to_hdr(x5,1,1/2.2)),1)

            # self.data_out = self.netTMRNet(x1, x2, x3, x4, x5)
...
class TMRNet(nn.Module):
    def __init__(self, opt, mid_channels=64, max_residue_magnitude=10):
        ...
        if opt.block == 'tmrnet':
            self.backbone['backward'] = ResidualBlocksWithInputConv(5 * mid_channels, mid_channels, 16)

            for i in range(0, 5):
                self.backbone['backward_rec_%d' % (i + 1)] = ResidualBlocksWithInputConv(1 * mid_channels, mid_channels,
                                                                                         24)


        if opt.block == 'Convnext':
            self.backbone['backward'] = ConvnextBlocksWithInputConv(5 * mid_channels, mid_channels, 1)

            for i in range(0, 2):
                self.backbone['backward_rec_%d' % (i + 1)] = ConvnextBlocksWithInputConv(1 * mid_channels, mid_channels,
                                                                                         1)
        self.reconstruction = ResidualBlocksWithInputConv(2 * mid_channels, mid_channels, 5)
        self.down = ResidualBlocksWithInputConv(3 * mid_channels, mid_channels, 3)
    ...
    def forward(self, lqs):
        ...
        base_feat = feats_[:, 0, :, :, :]
        feats_ = torch.cat((feats_[:, 0, :, :, :], feats_[:, 1, :, :, :], feats_[:, 2, :, :, :], feats_[:, 3, :, :, :],
                            feats_[:, 4, :, :, :]), 1)
        # feature propagation
        module = 'backward'
        feats0 = self.backbone[module](feats_)
        feats1 = self.backbone['backward_rec_%d' % (0 + 1)](feats0)
        feats2 = self.backbone['backward_rec_%d' % (1 + 1)](feats1)

        feats0 = torch.cat((feats0, feats1, feats2), 1)

        out = self.upsample(lqs, feats0, base_feat)
...
class ConvnextBlocksWithInputConv(nn.Module):
    def __init__(self, in_channels, out_channels=64, num_blocks=3):
        super().__init__()
        ...
        for i in range(num_blocks):
            main.append(
                basic_block(out_channels, out_channels, depth=10, ratio=1))
